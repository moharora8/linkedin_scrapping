...
def linkedin_login(user_name, password, url_entered):
	...
	print(login.prettify())

	# Get hidden form inputs
	#inputs = login.find('form', {'name': 'login'})
	inputs = login.find('body')
	print("----------------------------------------")
	print(inputs)
	#.find_all('input', {'type': ['hidden', 'submit']})

	# Create POST data
	post = {input.get('name'): input.get('value') for input in inputs}
	post['session_key'] = user_name
	post['session_password'] = password
	print(post['session_key'])
	print(post['session_password'])
	print(post)
	# Post login
	...
	print('--------------------------------------------')
	login23 = BeautifulSoup(home_response.text,'html.parser')
	print(login23.prettify())
...
def feature_extraction(home_response):
	...
	location_code_init = home_decoded_string.split(b'"patentView"')
